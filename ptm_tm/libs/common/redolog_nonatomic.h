...
///
/// Warning: This is the "nonatomic" version of the RedoLog.  It is incorrect
///          with respect to the C++ memory model, because a thread may write
///          back to an address at the same time as some concurrent doomed
///          transaction is reading from that address, and since they are not
///          *both* performing their access through a std::atomic<> variable, it
///          is racy.
template <int CHUNKSIZE> class RedoLog_Nonatomic {
...
public:
  ...
  RedoLog_Nonatomic(const size_t initial_capacity = 64)
  ...
  /// write the contents of the redolog back to main memory.  This version does
  /// worry about being atomic wrt the C++ memory model, but for
  /// redolog_nonatomic, we are ignoring this case, so we can forward to
  /// writeback_nonatomic.
  void writeback_atomic() { writeback_nonatomic(); }
  ...
  /// the redolog. Since this is the racy implementation of a redolog, the
