...
/// P_OrecLazy is an STM algorithm with the following characteristics:
...
///
/// P_OrecLazy can be customized in the following ways:
/// - Size of orec table
...
template <class ORECTABLE, class REDOLOG, class EPOCH, class CM,
          class STACKFRAME, class ALLOCATOR, class PMEM>
class P_OrecLazy {
...
public:
  ...
  template <typename T> void write(T *addr, T val) {
    ...
    // New chunk... need to read the whole chunk first.  We are very careful
    // here to avoid needing to validate.
    orec_t *o = globals.orecs.get(addr);
    local_orec_t pre, post;
    pre.all = o->curr;
...
private:
  ...
  /// If we locked the orec, we did so when the time was smaller than our
  /// start time, so we're sure to be OK.
  void validate() {
