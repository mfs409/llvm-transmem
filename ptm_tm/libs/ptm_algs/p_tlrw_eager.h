...
#include "../common/p_pipeline_log.h"
...
template <class BYTELOCKTABLE, class UNDOLOG, class EPOCH, class CM,
          ...
          int WRITE_TRIES, int WRITE_SPINS, bool PIPELINE_STORES, class PMEM>
class P_TLRWEager {
  ...
  /// value
  p_pipeline_log_t pipeline_store;
    ...
    if (frame.onEnd()) {
      ...
      if (PIPELINE_STORES) {
        if (pipeline_store.addr.p != nullptr) {
          ...
          PMEM::clwb(pipeline_store.addr.p);
          pipeline_store.addr.p = nullptr;
          need_fence = true;
        }
      }
