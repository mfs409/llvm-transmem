...
# Names of benchmark files / names of benchmarks.  We never split a ubench
...
# benchmark
UBENCH_NAMES = rbtree_bench list_bench hash_bench counter_bench empty_bench

# Get configuration info specific to the API that is being built right now. This
# will break if ubench.mk is used directly, because API needs to be defined.
# That's intentional.
...
EXEFILES       = $(foreach t, $(API_LIBS), $(ODIR)/$(UBENCH).$t.exe)
OFILES         = $(patsubst %, $(ODIR)/%.o, $(UBENCH_NAMES))
DEPS           = $(patsubst %.o, %.d, $(OFILES))
UBENCH_TARGETS = $(patsubst %, %.ubench, $(UBENCH_NAMES))
...
all: $(UBENCH_TARGETS)
%.ubench: $(ODIR)/%.o
	@UBENCH=$(basename $@) make linkstep -f $(BENCH_ROOT)/ubench.mk
linkstep: $(EXEFILES)
...
$(ODIR)/$(UBENCH).%.exe:
	@echo "[LD] $@"
	@$(LD) $(TM_ROOT)/libs/obj$(BITS)/$(basename $(subst $(ODIR)/$(UBENCH)., , $@)).o $(ODIR)/$(UBENCH).o -o $@ $(LDFLAGS)
