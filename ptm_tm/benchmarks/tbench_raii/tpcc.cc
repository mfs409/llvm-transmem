...
bool NewOrder(long wid, TpccBenchmark *tpcc, unsigned short seed[3],
              int orders) {
  char pad_begin[32];
  pad_begin[0] = 'a';
  int dcids[8];
...

  {
    ...
    DistrictRow *dr = (DistrictRow *)hash_table_read(
        tpcc->district_table, (uint64_t)DISTRICT_PID(w, d));
    ...
    for (long i = 0; i < orders; i++) {
      ...
      StockRow *sr =
          (StockRow *)hash_table_read(tpcc->stock_table, STOCK_PID(w, itemid));
      if (sr == NULL || sr->S_QUANTITY < num) {
        dcids[2] = 0;
        goto out; // get out of the transaction
      }
...
void *test(void *data) {
  ...
  while (done == 0) {
    // NB: we can adjust the size of tx here
