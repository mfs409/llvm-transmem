...
item_lock_count]; if (pthread_mutex_trylock(lock) == 0) { return lock;
    }
    return NULL;
}
 ...
 */
int is_listen_thread() { return pthread_self() == dispatcher_thread.thread_id; }
...
item *item_get(const char *key, const size_t nkey) {
  ...
  hv = hash(key, nkey, 0);
  // item_lock(hv);
  TX_BEGIN { it = do_item_get(key, nkey, hv); }
  TX_END;
  // item_unlock(hv);
  return it;
}
...
item *item_touch(const char *key, size_t nkey, uint32_t exptime) {
  ...
  hv = hash(key, nkey, 0);
  // item_lock(hv);
  TX_BEGIN { it = do_item_touch(key, nkey, exptime, hv); }
  TX_END;
  // item_unlock(hv);
  return it;
}
...
int item_link(item *item) {
  ...
  // item_lock(hv);
  TX_BEGIN { ret = do_item_link(item, hv); }
  TX_END;
  // item_unlock(hv);
  return ret;
}
...
void item_unlink(item *item) {
  ...
  hv = hash(ITEM_key(item), item->nkey, 0);
  // item_lock(hv);
  TX_BEGIN { do_item_unlink(item, hv); }
  TX_END;
  // item_unlock(hv);
}
...
enum delta_result_type add_delta(conn *c, const char *key, const size_t nkey,
  ...
  hv = hash(key, nkey, 0);
  // item_lock(hv);
  TX_BEGIN { ret = do_add_delta(c, key, nkey, incr, delta, buf, cas, hv); }
  TX_END;
  // item_unlock(hv);
  return ret;
}
...
enum store_item_type store_item(item *item, int comm, conn *c) {
  ...
  hv = hash(ITEM_key(item), item->nkey, 0);
  // item_lock(hv);
  TX_BEGIN { ret = do_store_item(item, comm, c, hv); }
  TX_END;
  // item_unlock(hv);
  return ret;
}
...
void item_flush_expired() {
  ...
  TX_BEGIN { do_item_flush_expired(); }
  TX_END;
...
char *item_cachedump(unsigned int slabs_clsid, unsigned int limit,
  ...
  // mutex_lock(&cache_lock);
  TX_BEGIN { ret = do_item_cachedump(slabs_clsid, limit, bytes); }
  TX_END;
  // mutex_unlock(&cache_lock);
  return ret;
}
...
void item_stats(ADD_STAT add_stats, void *c) {
  // mutex_lock(&cache_lock);
  TX_BEGIN { do_item_stats(add_stats, c); }
  TX_END;
  // mutex_unlock(&cache_lock);
}
...
void item_stats_totals(ADD_STAT add_stats, void *c) {
  // mutex_lock(&cache_lock);
  TX_BEGIN { do_item_stats_totals(add_stats, c); }
  TX_END;
  // mutex_unlock(&cache_lock);
}
...
void item_stats_sizes(ADD_STAT add_stats, void *c) {
  // mutex_lock(&cache_lock);
  TX_BEGIN { do_item_stats_sizes(add_stats, c); }
  TX_END;
  // mutex_unlock(&cache_lock);
}
