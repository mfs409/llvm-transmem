...
TX_SAFE static void do_slabs_free(void *ptr, const size_t size, unsigned int id);
...
TX_SAFE static void split_slab_page_into_freelist(char *ptr, const unsigned int id) {
    slabclass_t *p = &slabclass[id];
    int x;
...
TX_SAFE void slabs_adjust_mem_requested(unsigned int id, size_t old, size_t ntotal)
{
    //pthread_mutex_lock(&slabs_lock);
    TX_BEGIN {
    slabclass_t *p;
