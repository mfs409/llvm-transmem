...
// [pz] Provide a transaction-safe version of realloc
TX_SAFE void *tx_realloc(void *ptr, size_t size, size_t old_size) {
  void *oldp = ptr;
  void *newp = aligned_alloc(32, size);
  if (newp == NULL)
    return NULL;
...
// [pz] Provide a safe strncpy function that copies its argument to
...
bool conn_add_to_freelist(conn *c) {
  ...
  if (freecurr < freetotal) {
    ...
    conn **new_freeconns =
        (conn **)realloc(freeconns, sizeof(conn *) * newsize);
    if (new_freeconns) {
      freetotal = newsize;
...
static void conn_shrink(conn *c) {
  ...
  if (c->iovsize > IOV_LIST_HIGHWAT) {
    struct iovec *newbuf = (struct iovec *)realloc(
...
static int ensure_iov_space(conn *c) {
  ...
  if (c->iovused >= c->iovsize) {
    int i, iovnum;
    struct iovec *new_iov = (struct iovec *)realloc(
...
static int build_udp_headers(conn *c) {
  ...
  for (i = 0; i < c->msgused; i++) {
    ...
    *hdr++ = 0;
    assert((void *)hdr ==
           (caddr_t)c->msglist[i].msg_iov[0].iov_base + UDP_HEADER_SIZE);
  }
...
static void complete_nread_ascii(conn *c) {
  ...
  TX_BEGIN { c->thread->stats.slab_stats[it->slabs_clsid].set_cmds++; }
  TX_END;
  ...
  if (strncmp(ITEM_data(it) + it->nbytes - 2, "\r\n", 2) != 0) {
    ...
    case NREAD_SET:
      ...
      break;
    case NREAD_CAS:
      MEMCACHED_COMMAND_CAS(c->sfd, ITEM_key(it), it->nkey, it->nbytes, cas);
      break;
    }
#endif
...
static void complete_incr_bin(conn *c) {
  ...
  if (settings.verbose > 1) {
    ...
    fprintf(stderr, " %lld, %llu, %d\n", (long long)req->message.body.delta,
            (long long)req->message.body.initial, req->message.body.expiration);
  }
...
static void process_bin_touch(conn *c) {
  ...
  if (it) {
    ...
    MEMCACHED_COMMAND_TOUCH(c->sfd, ITEM_key(it), it->nkey, it->nbytes,
                            ITEM_get_cas(it));
...
static void process_bin_get(conn *c) {
  ...
  if (it) {
    ...
    MEMCACHED_COMMAND_GET(c->sfd, ITEM_key(it), it->nkey, it->nbytes,
                          ITEM_get_cas(it));
...
TX_SAFE static void append_bin_stats(const char *key, const uint16_t klen,
                                     const char *val, const uint32_t vlen,
                                     conn *c) {
  char *buf = c->stats.buffer + c->stats.offset;
  uint32_t bodylen = klen + vlen;
  protocol_binary_response_header header = {
      {(uint8_t)PROTOCOL_BINARY_RES, PROTOCOL_BINARY_CMD_STAT,
       (uint16_t)htons(klen), (uint8_t)PROTOCOL_BINARY_RAW_BYTES,
       (uint8_t)htonl(bodylen), (uint16_t)c->opaque}};
...
TX_SAFE static void append_ascii_stats(const char *key, const uint16_t klen,
                                       const char *val, const uint32_t vlen,
                                       conn *c) {
...
TX_SAFE static bool grow_stats_buf(conn *c, size_t needed) {
  ...
  if (nsize != c->stats.size) {
    char *ptr = (char *)tx_realloc(c->stats.buffer, nsize, c->stats.size);
    if (ptr) {
      c->stats.buffer = ptr;
      c->stats.size = nsize;
    } else {
...
TX_SAFE static void append_stats(const char *key, const uint16_t klen,
                                 const char *val, const uint32_t vlen,
                                 const void *cookie) {
...
static void init_sasl_conn(conn *c) {
  ...
  if (!c->sasl_conn) {
    int result = sasl_server_new("memcached", NULL,
                                 my_sasl_hostname[0] ? my_sasl_hostname : NULL,
                                 NULL, NULL, NULL, 0, &c->sasl_conn);
    if (result != SASL_OK) {
      if (settings.verbose) {
...
static void bin_list_sasl_mechs(conn *c) {
  ...
  int result = sasl_listmech(c->sasl_conn, NULL,
                             ...
                             &result_string, &string_length, NULL);
  if (result != SASL_OK) {
...
static void process_bin_complete_sasl_auth(conn *c) {
  ...
  // char mech[nkey+1];
  char *mech = (char *)malloc(sizeof(char) * nkey);
  ...
  case PROTOCOL_BINARY_CMD_SASL_AUTH:
    result =
        sasl_server_start(c->sasl_conn, mech, challenge, vlen, &out, &outlen);
    break;
  case PROTOCOL_BINARY_CMD_SASL_STEP:
    result = sasl_server_step(c->sasl_conn, challenge, vlen, &out, &outlen);
    break;
  default:
  ...
  switch (result) {
  case SASL_OK:
    write_bin_response(c, (char *)"Authenticated", 0, 0,
                       strlen("Authenticated"));
    // pthread_mutex_lock(&c->thread->stats.mutex);
    TX_BEGIN { c->thread->stats.auth_cmds++; }
    TX_END;
    // pthread_mutex_unlock(&c->thread->stats.mutex);
    break;
  case SASL_CONTINUE:
...
static void dispatch_bin_command(conn *c) {
  ...
  case PROTOCOL_BINARY_CMD_VERSION:
    if (extlen == 0 && keylen == 0 && bodylen == 0) {
      write_bin_response(c, (char *)VERSION, 0, 0, strlen(VERSION));
    } else {
      protocol_error = 1;
    }
    break;
...
static void process_bin_flush(conn *c) {
  ...
  // pthread_mutex_lock(&c->thread->stats.mutex);
  TX_BEGIN { c->thread->stats.flush_cmds++; }
  TX_END;
  // pthread_mutex_unlock(&c->thread->stats.mutex);
...
static void process_bin_delete(conn *c) {
  ...
  if (it) {
    ...
    if (cas == 0 || cas == ITEM_get_cas(it)) {
      ...
      // pthread_mutex_lock(&c->thread->stats.mutex);
      TX_BEGIN { c->thread->stats.slab_stats[it->slabs_clsid].delete_hits++; }
      TX_END;
      // pthread_mutex_unlock(&c->thread->stats.mutex);
      item_unlink(it);
      write_bin_response(c, NULL, 0, 0, 0);
    ...
    write_bin_error(c, PROTOCOL_BINARY_RESPONSE_KEY_ENOENT, 0);
    // pthread_mutex_lock(&c->thread->stats.mutex);
    TX_BEGIN { c->thread->stats.delete_misses++; }
    TX_END;
    // pthread_mutex_unlock(&c->thread->stats.mutex);
  }
}
 ...
 */
TX_SAFE enum store_item_type do_store_item(item *it, int comm, conn *c,
                                           const uint32_t hv) {
  ...
  if (old_it != NULL && comm == NREAD_ADD) {
  ...
  } else if (!old_it && (comm == NREAD_REPLACE || comm == NREAD_APPEND ||
                         comm == NREAD_PREPEND)) {
    ...
    if (old_it == NULL) {
      ...
      stored = NOT_FOUND;
      // pthread_mutex_lock(&c->thread->stats.mutex);
      TX_BEGIN { c->thread->stats.cas_misses++; }
      TX_END;
      // pthread_mutex_unlock(&c->thread->stats.mutex);
      ...
      // pthread_mutex_lock(&c->thread->stats.mutex);
      TX_BEGIN { c->thread->stats.slab_stats[old_it->slabs_clsid].cas_hits++; }
      TX_END;
      // pthread_mutex_unlock(&c->thread->stats.mutex);
...
void append_stat(const char *name, ADD_STAT add_stats, void *c, const char *fmt,
                 ...) {
  ...
  do {
    while (key_token->length != 0) {
      ...
      if (it) {
        if (i >= c->isize) {
          item **new_list =
              (item **)realloc(c->ilist, sizeof(item *) * c->isize * 2);
          if (new_list) {
            c->isize *= 2;
            c->ilist = new_list;
          } else {
        ...
        if (return_cas) {
          ...
          if (i >= c->suffixsize) {
            char **new_suffix_list = (char **)realloc(
                c->suffixlist, sizeof(char *) * c->suffixsize * 2);
            if (new_suffix_list) {
              c->suffixsize *= 2;
 ...
 */
TX_SAFE enum delta_result_type do_add_delta(conn *c, const char *key,
  ...
  if (res + 2 > it->nbytes || it->refcount != 1) { /* need to realloc */
    item *new_it;
    uint32_t flags;
    flags = tm_atoi((ITEM_suffix(it) + 1));
    new_it =
        do_item_alloc(ITEM_key(it), it->nkey, flags, it->exptime, res + 2, hv);
    if (new_it == 0) {
      do_item_remove(it);
      return EOM;
    }
    ...
    TX_BEGIN { ITEM_set_cas(it, (settings.use_cas) ? get_cas_id() : 0); }
    TX_END;
...
static enum try_read_result try_read_udp(conn *c) {
  ...
  if (res > 8) {
    ...
    // pthread_mutex_lock(&c->thread->stats.mutex);
    TX_BEGIN { c->thread->stats.bytes_read += res; }
    TX_END;
    // pthread_mutex_unlock(&c->thread->stats.mutex);
...
void do_accept_new_conns(const bool do_accept) {
  ...
  if (do_accept) {
    // STATS_LOCK();
    TX_BEGIN { stats.accepting_conns = true; }
    TX_END;
    // STATS_UNLOCK();
  } else {
    // STATS_LOCK();
...
static enum transmit_result transmit(conn *c) {
  ...
  if (c->msgcurr < c->msgused) {
    ...
    if (res > 0) {
      // pthread_mutex_lock(&c->thread->stats.mutex);
      TX_BEGIN { c->thread->stats.bytes_written += res; }
      TX_END;
      // pthread_mutex_unlock(&c->thread->stats.mutex);
  ...
  while (!stop) {
    ...
    case conn_new_cmd:
      ...
      if (nreqs >= 0) {
        ...
        // pthread_mutex_lock(&c->thread->stats.mutex);
        TX_BEGIN { c->thread->stats.conn_yields++; }
        TX_END;
        // pthread_mutex_unlock(&c->thread->stats.mutex);
        if (c->rbytes > 0) {
    ...
    case conn_nread:
      ...
      if (settings.verbose > 0) {
        fprintf(stderr,
                ...
                errno, strerror(errno), (long)c->rcurr, (long)c->ritem,
...
static void usage(void) {
  ...
  printf(
      ...
      "-l <addr>     interface to listen on (default: INADDR_ANY, all "
      "addresses)\n"
      "              <addr> may be specified as host:port. If you don't "
      "specify\n"
      "              a port number, the value you specified with -p or -U is\n"
      "              used. You may specify multiple addresses separated by "
      "comma\n"
      ...
      "-m <num>      max memory to use for items in megabytes (default: 64 "
      "MB)\n"
      ...
      "-vvv          extremely verbose (also print internal state "
      "transitions)\n"
  ...
  printf(
      ...
      "              the memory page size could reduce the number of TLB "
      "misses\n"
  ...
  printf("-R            Maximum number of requests per event, limits the "
         "number of\n"
  ...
  printf("-B            Binding protocol - one of ascii, binary, or auto "
         "(default)\n");
  printf("-I            Override the size of each slab page. Adjusts max item "
         "size\n"
  ...
  printf(
      ...
      "              - hashpower: An integer multiplier for how large the "
      "hash\n"
      "                table should be. Can be grown at runtime if not big "
      "enough.\n"
...
int main(int argc, char **argv) {
  ...
  char *subopts;
  char *subopts_value;
  enum { MAXCONNS_FAST = 0, HASHPOWER_INIT, SLAB_REASSIGN, SLAB_AUTOMOVE };
  char *const subopts_tokens[] = {(char *)"maxconns_fast", (char *)"hashpower",
                                  (char *)"slab_reassign",
                                  (char *)"slab_automove", NULL};
