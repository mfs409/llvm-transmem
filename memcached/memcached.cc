...
static void process_bin_complete_sasl_auth(conn *c) {
    ...
    switch(result) {
    case SASL_OK:
        write_bin_response(c, (char *)"Authenticated", 0, 0, strlen("Authenticated"));
        //pthread_mutex_lock(&c->thread->stats.mutex);
        TX_BEGIN {
 ...
 */
TX_SAFE enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
 ...
 */
TX_SAFE enum delta_result_type do_add_delta(conn *c, const char *key, const size_t nkey,
