...
void initialize_private_node_pool(uint64_t total_threads,
  ...
  t_nonleaves =
      (bplus_non_leaf *)aligned_alloc(32, sizeof(bplus_non_leaf) * local_nodes);
}
...
#else
  struct bplus_non_leaf
      *node = //(struct bplus_non_leaf*)malloc(sizeof(struct bplus_non_leaf));
...
#else
  struct bplus_leaf
      *node = //(struct bplus_leaf*)malloc(sizeof(struct bplus_leaf));
...
long non_leaf_insert(struct bplus_tree *tree, struct bplus_non_leaf *node,
                     struct bplus_node *sub_node, long key, long level) {
  long i, j, split_key;
  long split = 0;
  ...
  if (split) {
    if (tree->head[level + 1] == nullptr) {
      if (++level >= tree->level) {
        // fprintf(stderr, "!!Panic: Level exceeded, please expand the tree
        // level, non-leaf order or leaf entries for element capacity!\n");
      ...
      return non_leaf_insert(tree, node->parent, (struct bplus_node *)sibling,
                             split_key, level + 1);
    }
  }
...
long leaf_insert(struct bplus_tree *tree, struct bplus_leaf *leaf, long key,
                 long data) {
  long i, j, split = 0;
  ...
  if (split) {
    ...
    if (parent == nullptr) {
      ...
      /* trace upwards */
      sibling->parent = parent;
      return non_leaf_insert(tree, parent, (struct bplus_node *)sibling,
                             sibling->key[0], 1);
    }
  }
...
long bplus_tree_insert(bplus_tree *tree, long key, long data) {
  long i;
  struct bplus_node *node = tree->root;
  struct bplus_non_leaf *nln;
...
void non_leaf_remove(struct bplus_tree *tree, struct bplus_non_leaf *node,
                     long remove, long level) {
  long i, j, k;
  struct bplus_non_leaf *sibling;
  ...
  if (node->children <= (tree->order + 1) / 2) {
    ...
    if (parent != nullptr) {
      ...
      if (i == 0) {
        ...
        struct bplus_non_leaf *l_sib =
            (struct bplus_non_leaf *)parent->sub_ptr[i - 1];
        ...
        borrow = l_sib->children >= r_sib->children ? BORROW_FROM_LEFT
                                                    : BORROW_FROM_RIGHT;
      }
      ...
      if (borrow == BORROW_FROM_LEFT) {
        ...
        if (sibling->children > (tree->order + 1) / 2) {
          ...
          for (j = node->children - 1, k = 0; k < sibling->children - 1;
               j++, k++) {
    ...
    if (parent != nullptr) {
      ...
      if (i >= 0) {
        ...
        if (i == parent->children - 1) {
          ...
          struct bplus_leaf *l_sib =
              (struct bplus_leaf *)parent->sub_ptr[i - 1];
          struct bplus_leaf *r_sib =
              (struct bplus_leaf *)parent->sub_ptr[i + 1];
          /* if both left and right sibling found, choose the one with more
           * entries */
          sibling = l_sib->entries >= r_sib->entries ? l_sib : r_sib;
          borrow = l_sib->entries >= r_sib->entries ? BORROW_FROM_LEFT
                                                    : BORROW_FROM_RIGHT;
        }
      } else {
        i = -i - 1;
      ...
      if (borrow == BORROW_FROM_LEFT) {
        ...
        /* remove element first in case of overflow during merging with sibling
         * node */
...
long bplus_tree_size(struct bplus_tree *tree) {
  long i = 0;
  struct bplus_leaf *leaf = (struct bplus_leaf *)tree->head[0];
  while (leaf != nullptr) {
    i++;
...
long bplus_tree_get(struct bplus_tree *tree, long key) {
  long data = bplus_tree_search(tree, key);
  return data;
}

long bplus_tree_put(struct bplus_tree *tree, long key, long data) {
  return bplus_tree_insert(tree, key, data);
}

struct bplus_tree *bplus_tree_init(long level, long order, long entries) {
  ...
  struct bplus_tree *tree =
      (struct bplus_tree *)aligned_alloc(32, sizeof(struct bplus_tree));
  tree->root = nullptr;
  tree->level = level;
  tree->order = order;
