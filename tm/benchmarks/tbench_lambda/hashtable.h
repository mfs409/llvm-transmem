...
Hashtable *hash_table_init(uint64_t size) {
  ...
  HashtableItem *items =
      (HashtableItem *)aligned_alloc(32, sizeof(HashtableItem) * size);
...
TX_SAFE uint64_t hash_table_read(struct Hashtable *hashtable, uint64_t _key) {
  ...
  for (int l = 0; l < item->_top; l++) {
    if (item->_keys[l] == _key)
      return item->_value[l];
  }
  return 0;
}

long hash_table_account(Hashtable *hashtable) { return 0; }