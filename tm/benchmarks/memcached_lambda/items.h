...
do_item_link(item *it,
             const uint32_t hv); /** may fail if transgresses limits */
...
TX_SAFE void
do_item_update(item *it); /** update LRU time to current and reposition */
...
TX_SAFE item *do_item_get(const char *key, const size_t nkey,
                          const uint32_t hv);
TX_SAFE item *do_item_touch(const char *key, const size_t nkey,
                            uint32_t exptime, const uint32_t hv);
