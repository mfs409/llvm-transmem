...
template <class UNDOLOG, class EPOCH, class CM, class STACKFRAME,
          class ALLOCATOR>
class TMLEager {
...
public:
  ...
  template <typename T> T read(T *ptr) {
    ...
    T from_mem = UNDOLOG::perform_transactional_read(ptr);
    if ((lock_snapshot & 1) == 0) {
  ...
  template <typename T> void write(T *ptr, T val) {
    if (accessDirectly(ptr)) {
      // accessDirectly() returns true for stack accesses and also when this
      // thread already holds the lock.  In the former case, we could just do
      // *ptr = val.  In the latter case, we need to use C++ atomics.  For
      // simplicity, we just use atomics every time.
      UNDOLOG::perform_transactional_write(ptr, val);
    } else {
