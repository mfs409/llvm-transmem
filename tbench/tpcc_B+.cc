...
TpccBenchmarkBPlusTree* init_tpcc_tree(){
  TpccBenchmarkBPlusTree* tpcc = (TpccBenchmarkBPlusTree*)malloc(sizeof(TpccBenchmarkBPlusTree));
...
bool NewOrder(long wid, TpccBenchmarkBPlusTree* tpcc, unsigned short seed[3], int orders){
  ...
  TX_BEGIN {
    ...
    DistrictRow* dr = (DistrictRow *)bplus_tree_get(tpcc->district_table, (long)(DISTRICT_PID(w, d)));
    ...
    bplus_tree_put(tpcc->new_order_table, (long)key, (long)norow);   // NewOrder Table
    bplus_tree_put(tpcc->order_table, (long)key, (long)orow);        // Order Table
    // Order Line
    for(long i = 0; i < orders; i++){
      long num = item_nums[i];
      long itemid = item_ids[i];
      StockRow* sr = (StockRow*)bplus_tree_get(tpcc->stock_table, STOCK_PID(w, itemid));
