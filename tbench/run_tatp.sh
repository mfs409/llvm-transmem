...
if [ -f tatp_f1.txt ] ; then
    rm tatp_f1.txt
fi

if [ -f tatp_f2.txt ] ; then
    rm tatp_f2.txt
fi
...
do
    for t in $TMLIB
    do
        echo "Running TATP with $t and $i threads ..."
        numactl -N 0 ./obj64/tatp_$t -d 5 -t $i >> tatp_f1.txt
        echo -n "," >> tatp_f1.txt
    done
    echo -n -e "\n"  >> tatp_f1.txt
done
...
do
    ...
    do
        echo "Running TATP with $t and $i threads ..."
        numactl -N 0 ./obj64/tatp_$t -d 5 -t $i >> tatp_f2.txt
        echo -n "," >> tatp_f2.txt
    done
    echo -n -e "\n"  >> tatp_f2.txt
done
