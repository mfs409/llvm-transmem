...
if [ -f tpcc_f1.txt ] ; then
    rm tpcc_f1.txt
fi

if [ -f tpcc_f2.txt ] ; then
    rm tpcc_f2.txt
fi
...
do
    for t in $TMLIB
    do
        echo "Running TPCC (Hashtable) with $t and $i threads ..."
        numactl -N 0 ./obj64/tpcc_$t -d 5 -t $i >> tpcc_f1.txt
        echo -n "," >> tpcc_f1.txt
    done
    echo -n -e "\n"  >> tpcc_f1.txt
done
...
do
    ...
    do
        echo "Running TPCC (Hashtable) with $t and $i threads ..."
        numactl -N 0 ./obj64/tpcc_$t -d 5 -t $i >> tpcc_f2.txt
        echo -n "," >> tpcc_f2.txt
    done
    echo -n -e "\n"  >> tpcc_f2.txt
done
