...
template <class T> class P_MiniVector {
  ...
  void p_expand() {
    ...
    for (int i = 0; i < capacity; i += CACHELINE_BYTES) {
      pmem_clwb(temp + i);
    }
    pmem_sfence();
...
public:
  ...
  P_MiniVector(const unsigned long _capacity = 64)
    ...
    // recover
    pmem_clwb(&p_timestamp);
    // Get memory, zero it, flush it
    char *items_array =
        (char *)aligned_alloc(CACHELINE_BYTES, sizeof(p_entry_t) * capacity);
    memset(items_array, 0, sizeof(p_entry_t) * capacity);
    for (int i = 0; i < sizeof(p_entry_t) * capacity; i += CACHELINE_BYTES) {
      pmem_clwb(items_array + i);
    }
    pmem_sfence();
