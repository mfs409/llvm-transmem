...
#define CREATE_FUNC_1(NAME, RETTY, ARGSTY1)                                    \
  cast<Function>(M.getOrInsertFunction(                                        \
      NAME, FunctionType::get(RETTY, {ARGSTY1}, false)));
...
void signatures::init(Module &M) {
  ...
  this->funcs[MEMCPY] = cast<Function>(M.getOrInsertFunction(
      TM_MEMCPY_STR,
      FunctionType::get(types[I8P],
                        {types[I8P], types[I8P], types[I64], types[I32]},
                        false)));
  this->funcs[MEMSET] = cast<Function>(M.getOrInsertFunction(
      TM_MEMSET_STR,
      FunctionType::get(
          types[I8P], {types[I8P], types[I8], types[I64], types[I32]}, false)));
  this->funcs[MEMMOVE] = cast<Function>(M.getOrInsertFunction(
      TM_MEMMOVE_STR,
  ...
  funcs[UNSAFE] = cast<Function>(M.getOrInsertFunction(
      TM_UNSAFE_STR,
  ...
  if (Function *OriginalFunc = M.getFunction(TM_EXECUTE_C_STR)) {
    ...
    funcs[CAPI] = cast<Function>(M.getOrInsertFunction(
        TM_EXECUTE_C_INTERNAL_STR, FuncTy, OriginalFunc->getAttributes()));
  }
}
